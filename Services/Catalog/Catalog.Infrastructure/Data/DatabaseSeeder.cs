...
namespace Catalog.Infrastructure.Data
{
    public class DatabaseSeeder
    {
        public static async Task SeedAsync(IOptions<DatabaseSettings> options)
        {
            ...
            {
                var brandData = await File.ReadAllBytesAsync(Path.Combine(SeedBasePath, "brand.json"));
            ...
            {
                var typeData = await File.ReadAllBytesAsync(Path.Combine(SeedBasePath, "types.json"));
